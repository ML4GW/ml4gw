...
def test_chirp_time_bound(masses, spins, fstart):
    ...
    for i, (fs, m1_val, m2_val, s1_val, s2_val) in enumerate(
        zip(fstart, m1, m2, s1, s2, strict=True)
    ):
...
def test_final_black_hole_spin_bound(spins):
    ...
    for i, (s1_val, s2_val) in enumerate(zip(s1, s2, strict=True)):
...
def test_merge_time_bound(masses):
    ...
    for i, (m1_val, m2_val) in enumerate(zip(m1, m2, strict=True)):
