...
class MinimumPhaseWhitenTest:
    ...
    def get_asds(self):
        ...
        for coefficient in (0.2, 0.8):
            ...
            asds.append((2 / self.sample_rate) ** 0.5 / response.abs())
        return asds
...
class TestMinimumPhaseWhiten(MinimumPhaseWhitenTest):
    ...
    def test_crop_removes_settle_in(self):
        ...
        asd = torch.full((num_freqs,), (2 / self.sample_rate) ** 0.5)
        X = torch.randn(4, self.num_channels, 1024)

        uncropped = transform(X, asd, crop=False)
        whitened = transform(X, asd)
    ...
    @pytest.mark.parametrize("asd_ndim", [2, 3])
    def test_asd_broadcasting(self, asd_ndim):
        transform = self.get_transform()
        X = torch.randn(4, self.num_channels, 1024)
        asd = torch.stack(self.get_asds())
        if asd_ndim == 3:
        ...
        whitened = transform(X, asd, crop=False)
    ...
    def test_matches_standard_frequency_response(self, highpass, lowpass):
        ...
        kernel = transform(impulse, asd, crop=False)[0, 0]
        response = torch.fft.rfft(kernel).abs()
        truncated_asd = spectral.truncate_inverse_amplitude_spectrum(
            asd[None, None],
            self.fduration,
            self.sample_rate,
            highpass,
        ...
        expected = truncated_asd.reciprocal() / self.sample_rate**0.5
    ...
    def test_realistic_aligo_asd_at_2048_hz(self):
        sample_rate = 2048
        fduration = 2
        n_fft = 8 * sample_rate
        num_freqs = n_fft // 2 + 1
        ...
        asd = torch.from_numpy(psd.data.data.copy()).sqrt()
        transform = MinimumPhaseWhiten(
            fduration,
            sample_rate,
            highpass=20,
        ...
        expected = truncated_asd.reciprocal() / sample_rate**0.5
    ...
    def test_validation(self):
        ...
        with pytest.raises(ValueError, match="one, two, or three dimensions"):
            transform(X, asd.repeat(2, 2, 2, 1))
        with pytest.raises(ValueError, match="channels"):
            transform(X, asd.repeat(self.num_channels + 1, 1))
        with pytest.raises(ValueError, match="batch size"):
            transform(X, asd.repeat(X.size(0) + 1, self.num_channels, 1))
        with pytest.raises(ValueError, match="Not enough timeseries"):
            transform(X[..., : self.size - 1], asd)
        with pytest.raises(ValueError, match="Not enough timeseries"):
            transform(X[..., : self.size], asd)
    ...
    def test_upsamples_short_asd_before_factorization(self):
        transform = self.get_transform()
        asd = torch.full((self.size // 2 + 1,), (2 / self.sample_rate) ** 0.5)
        ...
        factorized_asd = factorize.call_args.args[0]
        assert factorized_asd.size(-1) == self.size + 1
...
class TestFixedMinimumPhaseWhiten(MinimumPhaseWhitenTest):
    ...
    def test_whitens_first_order_colored_noise(self):
        ...
        omega = 2 * torch.pi * torch.arange(n_fft // 2 + 1) / n_fft
        response = 1 - coefficient * torch.exp(-1j * omega)
        asd = (2 / self.sample_rate) ** 0.5 / response.abs()
        transform.fit(asd, asd)
    ...
    def test_fit_preserves_resolved_asd_grid(self):
        transform = self.get_transform()
        asd = torch.ones(8 * self.size + 1, dtype=torch.float64)
        asd[257:265] = 100
        ...
        fitted_asd = factorize.call_args.args[0]
        assert fitted_asd.size(-1) == asd.size(-1)
    ...
    def test_truncated_filter_matches_standard_resolved_line(self):
        ...
        whitened_asd = response.abs() * asd * (self.sample_rate / 2) ** 0.5
        truncated_asd = spectral.truncate_inverse_amplitude_spectrum(
            asd[None, None],
            self.fduration,
            self.sample_rate,
        )[0, 0]
        expected = asd / (2**0.5 * truncated_asd)
        band = (frequencies >= 5) & (frequencies <= 120)
        log_error = whitened_asd[band].log().abs()
    ...
    def test_fit_bandpass_matches_dynamic_transform(self):
        ...
        transform = self.get_transform()
        transform.fit(asd, asd, highpass=highpass, lowpass=lowpass)
    ...
    def test_preserves_fitted_precision_internally(self):
        ...
        asd = torch.ones(2 * self.size + 1, dtype=torch.float64)
        transform.fit(asd, asd)
