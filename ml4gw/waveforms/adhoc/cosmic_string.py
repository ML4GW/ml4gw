...
class GenerateString(torch.nn.Module):
    ...
    def forward(
        self, power: float, amplitude: BatchTensor, f_high: BatchTensor
    ) -> (Tensor, Tensor):
        """
        ...
        """
        # ToDo:
        # Better add a tye check to the input variables,
        # since it's mostly comes from sampled values
        ...
        ratio = freq / f_high.view(-1, 1)  # (batch, freq_bins)
        taper = torch.where(
