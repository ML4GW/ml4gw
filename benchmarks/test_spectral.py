...
def test_spectral_density(benchmark, batch_size, num_segments, device):
    window = torch.hann_window(NUM_SAMPLES, device=device)
    x = torch.randn(
        batch_size, NUM_CHANNELS, num_segments * NUM_SAMPLES, device=device
    )
    benchmark(
        spectral.spectral_density, x, NUM_SAMPLES, NUM_SAMPLES, window, SCALE
    )


def test_truncate_inverse_power_spectrum(benchmark, device):
...
def test_normalize_by_psd(benchmark, batch_size, device):
    n = NUM_SAMPLES * 4
    ...
    benchmark(spectral.normalize_by_psd, x, psd, SAMPLE_RATE, pad)


def test_whiten(benchmark, batch_size, device):
    n = NUM_SAMPLES * 4
    x = torch.randn(batch_size, NUM_CHANNELS, n, device=device)
    num_freqs = n // 2 + 1
    psd = torch.rand(NUM_CHANNELS, num_freqs, device=device) + 1e-20
    ...
    benchmark(spectral.whiten, x, psd, fduration, SAMPLE_RATE)
