...
def test_waveform_sampler_forward(benchmark, batch_size):
    n = batch_size * 2
    plus = torch.randn(n, NUM_SAMPLES)
    cross = torch.randn(n, NUM_SAMPLES)
    sampler = WaveformSampler(plus=plus, cross=cross)
    benchmark(sampler, batch_size)


def test_waveform_projector_forward(benchmark, batch_size, device):
    ...
    benchmark(projector, dec, psi, phi, plus=plus, cross=cross)
