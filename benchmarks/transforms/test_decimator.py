...
def test_decimator_forward(benchmark, batch_size, device, maybe_sync):
    ...
    benchmark(maybe_sync(decimator), x)


def test_decimator_split_forward(benchmark, batch_size, device, maybe_sync):
    schedule = torch.tensor([[0, 1, 256]], dtype=torch.int)
    decimator = Decimator(
